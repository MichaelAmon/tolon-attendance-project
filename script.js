     ...
     async function startLocationWatch() {
       ...
       async function validateFace(imageData) {
         ...
         try {
           ...
           console.log('CompreFace Response:', { status: response.status, text: text.slice(0, 100) });
           const result = JSON.parse(text);
           if (result.result && result.result.length > 0 && result.result[0].similarity > 0.6) {
             return { success: true, name: result.result[0].subject };
           }
           return { success: false, error: 'No matching face or low similarity' };
         } catch (error) {
           console.error('CompreFace error:', error.message, url);
           return { success: false, error: `Failed to fetch CompreFace: ${error.message}` };
         }
       }
       ...
       async function captureAndCompare() {
         ...
         return await validateFace(imageData);
       }
         ...
         setTimeout(async () => {
           ...
           if (result.success && result.name) {
             ...
             try {
               ...
               const text = await response.text();
               console.log('Attendance Response:', { status: response.status, text: text.slice(0, 100) });
